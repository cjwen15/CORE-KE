...
if os.name == 'nt':
    bert_path = "\\media\\wen\\d_1T\\bert_bilstm_crf\\pretraining_output"
    root_path = r'\media\wen\d_1T\bert_bilstm_crf'
else:
    bert_path = '/media/wen/d_1T/bert_bilstm_crf/pretraining_output'
    root_path = '/media/wen/d_1T/bert_bilstm_crf'
...
flags.DEFINE_string(
    "task_name", 'ehrscner', "The name of the task to train."
)
...
## Other parameters
flags.DEFINE_string(
    "init_checkpoint", os.path.join(bert_path, 'model.ckpt-500000'),
...
flags.DEFINE_bool("do_train", True, "Whether to run training.")  # True

flags.DEFINE_bool("use_tpu", False, "Whether to use TPU or GPU/CPU.")

flags.DEFINE_bool("do_eval", False, "Whether to run eval on the dev set.")  # True

flags.DEFINE_bool("do_predict", False, "Whether to run the model in inference mode on the test set.")  # True

flags.DEFINE_integer("train_batch_size", 8, "Total batch size for training.")  # 16

flags.DEFINE_integer("eval_batch_size", 8, "Total batch size for eval.")  # 8

flags.DEFINE_integer("predict_batch_size", 8, "Total batch size for predict.")  # 8

flags.DEFINE_float("learning_rate", 5e-5, "The initial learning rate for Adam.")  # 5e-5

flags.DEFINE_float("num_train_epochs", 30.0, "Total number of training epochs to perform.")  # 3.0
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_ids, ):  # label_mask原来是没有的
...
class EHRs_CNERProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
        return self._create_example(
            self._read_data(os.path.join(data_dir, "fine-tune_text_DSNF.txt")), "train"
        )
    ...
    def model_fn(features, labels, mode, params):
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(label_ids, logits, trans):
                ...
                precision = tf_metrics.precision(label_ids, pred_ids, num_labels, [1, 2, 3, 4, 5, 6], average="macro") # weight
                recall = tf_metrics.recall(label_ids, pred_ids, num_labels, [1, 2, 3, 4, 5, 6], average="macro")
                f = tf_metrics.f1(label_ids, pred_ids, num_labels, [1, 2, 3, 4, 5, 6], average="macro")
    ...
    if FLAGS.do_predict:
        ...
        with open(output_predict_file, "w", encoding="utf-8") as f:
            for example, prediction in zip(predict_examples, result):
                item_id = 0
                line = ""
                line_token = str(example.text).split(' ')
                label_token = str(example.label).split(' ')
                if len(line_token) != len(label_token):
                    tf.logging.info(example.text)
